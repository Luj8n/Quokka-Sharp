...
class TestSimulation:
    ...
    def test_identity_allzero(self):
        ...
        p = qk.functionalities.sim(
            qasm("single_qubit", "identity_wire.qasm"),
            basis="comp",
            measurement="allzero",
        )
        assert_prob(p, 1.0)
    ...
    def test_h_allzero(self):
        # H|0> = |+>  →  P(0) = 0.5
        p = qk.functionalities.sim(
            qasm("single_qubit", "h.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 0.5)
    ...
    def test_h_firstzero(self):
        ...
        p = qk.functionalities.sim(
            qasm("single_qubit", "h.qasm"), basis="comp", measurement="firstzero"
        )
        assert_prob(p, 0.5)
    ...
    def test_x_allzero(self):
        # X|0> = |1>  →  P(0) = 0.0
        p = qk.functionalities.sim(
            qasm("single_qubit", "x.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 0.0)
    ...
    def test_x_dict_one(self):
        # X|0> = |1>  →  P(q0=1) = 1.0
        p = qk.functionalities.sim(
            qasm("single_qubit", "x.qasm"), basis="comp", measurement={0: 1}
        )
        assert_prob(p, 1.0)
    ...
    def test_y_allzero(self):
        # Y|0> = i|1>  →  P(0) = 0.0
        p = qk.functionalities.sim(
            qasm("single_qubit", "y.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 0.0)
    ...
    def test_z_allzero(self):
        # Z|0> = |0>  →  P(0) = 1.0
        p = qk.functionalities.sim(
            qasm("single_qubit", "z.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 1.0)
    ...
    def test_s_allzero(self):
        # S|0> = |0>  →  P(0) = 1.0
        p = qk.functionalities.sim(
            qasm("single_qubit", "s.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 1.0)
    ...
    def test_t_allzero(self):
        # T|0> = |0>  →  P(0) = 1.0
        p = qk.functionalities.sim(
            qasm("single_qubit", "t.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 1.0)
    ...
    def test_hh_allzero(self):
        # HH = I  →  P(0) = 1.0
        p = qk.functionalities.sim(
            qasm("single_qubit", "hh.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 1.0)
    ...
    def test_hzh_allzero(self):
        # HZH = X  →  P(0) = 0.0
        p = qk.functionalities.sim(
            qasm("single_qubit", "hzh.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 0.0)
    ...
    def test_xxxx_allzero(self):
        # XXXX = I  →  P(0) = 1.0
        p = qk.functionalities.sim(
            qasm("single_qubit", "xxxx.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 1.0)
    ...
    def test_h_pauli(self):
        p = qk.functionalities.sim(
            qasm("single_qubit", "h.qasm"), basis="pauli", measurement="allzero"
        )
        assert_prob(p, 0.5)

    def test_x_pauli(self):
        p = qk.functionalities.sim(
            qasm("single_qubit", "x.qasm"), basis="pauli", measurement="allzero"
        )
        assert_prob(p, 0.0)
    ...
    def test_bell_p00(self):
        # Bell: P(00) = 0.5
        p = qk.functionalities.sim(
            qasm("multi_qubit", "bell.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 0.5)
    ...
    def test_bell_p11(self):
        # Bell: P(11) = 0.5
        p = qk.functionalities.sim(
            qasm("multi_qubit", "bell.qasm"), basis="comp", measurement={0: 1, 1: 1}
        )
        assert_prob(p, 0.5)
    ...
    def test_bell_p01(self):
        # Bell: P(01) = 0.0
        p = qk.functionalities.sim(
            qasm("multi_qubit", "bell.qasm"), basis="comp", measurement={0: 0, 1: 1}
        )
        assert_prob(p, 0.0)
    ...
    def test_bell_probabilities_sum_to_one(self):
        ...
        for b0 in (0, 1):
            for b1 in (0, 1):
                p = qk.functionalities.sim(
                    qasm("multi_qubit", "bell.qasm"),
                    basis="comp",
                    measurement={0: b0, 1: b1},
                )
    ...
    def test_ghz_p000(self):
        # GHZ: P(000) = 0.5
        p = qk.functionalities.sim(
            qasm("multi_qubit", "ghz_3q.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 0.5)
    ...
    def test_ghz_p111(self):
        # GHZ: P(111) = 0.5
        p = qk.functionalities.sim(
            qasm("multi_qubit", "ghz_3q.qasm"),
            basis="comp",
            measurement={0: 1, 1: 1, 2: 1},
        )
        assert_prob(p, 0.5)
    ...
    def test_cx_allzero(self):
        ...
        p = qk.functionalities.sim(
            qasm("multi_qubit", "cx_q0q1.qasm"), basis="comp", measurement="allzero"
        )
        assert_prob(p, 1.0)
    ...
    def test_cx_x_cx_result(self):
        ...
        p = qk.functionalities.sim(
            qasm("multi_qubit", "cx_x_cx.qasm"), basis="comp", measurement={0: 0, 1: 1}
        )
        assert_prob(p, 1.0)
    ...
    def test_return_is_float_or_status(self):
        result = qk.functionalities.sim(
            qasm("single_qubit", "h.qasm"), basis="comp", measurement="allzero"
        )
        assert isinstance(result, (float, decimal.Decimal)) or result in (
            "TIMEOUT",
            "MEMOUT",
        )
...
class TestVerification:
    ...
    def test_cx_control0_pass(self):
        ...
        res = qk.functionalities.verify(
            qasm("verify", "v03_cx_control0.qasm"),
            basis="comp",
            precons={0: 0, 1: 0},
            postcons={0: 0, 1: 0},
        )
    ...
    def test_cx_control1_pass(self):
        ...
        res = qk.functionalities.verify(
            qasm("verify", "v04_cx_control1.qasm"),
            basis="comp",
            precons={0: 0, 1: 0},
            postcons={0: 1, 1: 1},
        )
    ...
    def test_toffoli_no_flip(self):
        ...
        res = qk.functionalities.verify(
            qasm("verify", "v05_toffoli_no_flip.qasm"),
            basis="comp",
            precons={0: 0, 1: 0, 2: 0},
            postcons={0: 0, 1: 0, 2: 0},
        )
        skip_if_solver_issue(res)
        assert res in ("True", True)
    ...
    def test_toffoli_flip(self):
        ...
        res = qk.functionalities.verify(
            qasm("verify", "v06_toffoli_flip.qasm"),
            basis="comp",
            precons={0: 0, 1: 0, 2: 0},
    ...
    def test_xx_identity_pass(self):
        # XX = I
        res = qk.functionalities.verify(
            qasm("verify", "v07_xx_identity.qasm"),
            basis="comp",
            precons={0: 0},
            postcons={0: 0},
        )
    ...
    def test_cx_swap_target_pass(self):
        res = qk.functionalities.verify(
            qasm("verify", "v08_cx_swap_target.qasm"),
            basis="comp",
            precons={0: 0, 1: 0},
            postcons={0: 1, 1: 1},
        )
    ...
    def test_multi_gate_chain_pass(self):
        # X H H X = I
        res = qk.functionalities.verify(
            qasm("verify", "v10_multi_gate_chain.qasm"),
            basis="comp",
            precons={0: 0},
            postcons={0: 0},
        )
...
class TestSynthesis:
    ...
    def test_syn_structure(self, target, gate_set):
        ...
        result = qk.functionalities.syn(
            qasm("synthesis", target),
            basis="pauli",
            files_root="tmp",
            fid=1.0,
            cyc_lin_encoding=True,
            gate_set=gate_set,
        )
        assert isinstance(result, tuple) and len(result) == 4
        outcome, weight, qasm_str, layers = result
        valid = {"FOUND", "TIMEOUT", "CRASH"}
        assert outcome in valid or outcome.startswith("ERROR"), (
    ...
    def test_found_weight_is_one(self):
        ...
        outcome, weight, _, _ = qk.functionalities.syn(
            qasm("synthesis", "syn01_h_target.qasm"),
            basis="pauli",
            fid=1.0,
            files_root="tmp",
            gate_set={"h", "cx", "s"},
    ...
    def test_synthesised_circuit_is_equivalent(self):
        ...
        try:
            res = qk.functionalities.eq(
                target, syn_path, basis="comp", check="cyclic", epsilon=0
            )
            skip_if_solver_issue(res)
            assert res in (True, "True"), (
                "Synthesised circuit is not equivalent to target"
            )
        finally:
            os.unlink(syn_path)
    ...
    def test_2q_synthesis_structure(self):
        ...
        result = qk.functionalities.syn(
            qasm("synthesis", "syn04_cx_target.qasm"),
            basis="pauli",
            fid=1.0,
            files_root="tmp",
            gate_set={"h", "cx", "s"},
...
class TestEdgeCases:
    ...
    def test_invalid_basis_raises(self):
        with pytest.raises(Exception):
            qk.functionalities.sim(
                qasm("single_qubit", "h.qasm"), basis="bad", measurement="allzero"
            )
    ...
    def test_bell_psi_minus_no_allzero(self):
        ...
        p = qk.functionalities.sim(
            qasm("multi_qubit", "bell_psi_minus.qasm"),
            basis="pauli",
            measurement="allzero",
        )
        assert_prob(p, 0.0)
