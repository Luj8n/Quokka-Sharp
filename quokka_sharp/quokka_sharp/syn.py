...
def identity_check(cnf: "CNF", cnf_file_root, files_prefix, indx, onehot_xz=False):
    ...
    cnf_file = os.path.join(
        cnf_file_root, f"{files_prefix}_{indx}_quokka_syn.cnf"
    )  # overide files to reduce spaming
...
def Synthesis(
    ...
    if printing:
        print(
            f"basis:{'Comp' if cnf.computational_basis else 'Pauli'}, exact:{onehot_xz}"
        )
    if printing:
        print(f"fidelity_threshold:{fidelity_threshold}")
    p = None

    def cleanup():
        # Called on timeout to kill the process
        if printing:
            print(f"TIMEOUT expiered")
        if p is not None:
            p.kill()
    ...
    try:
        with timeout(TIMEOUT, on_timeout=cleanup):
            ...
            while not done:
                ...
                files_prefix = (
                    (
                        ("onehotXZ" if onehot_xz else "fullP")
                        if not cnf.computational_basis
                        else "comp"
                    )
                    + "_"
                    + ("HSan" if h_sandwich else "Reg")
                )
                ...
                if bin_search:
                    ...
                    cnf_copy.add_syn_layer(
                        gate_set, num_layers, limit_gates=h_sandwich, h_layer=False
                    )
                    cnf_copy.add_syn_layer(
                        gate_set, 1, limit_gates=h_sandwich, h_layer=True
                    )
                    file = identity_check(
                        cnf_copy,
                        cnf_file_root,
                ...
                command = (
                    tool_invocation.split(" ")
                    + ["-i", file]
                    + ["--complex", ("1" if cnf.computational_basis else "0")]
                    + [
                        "--threshold",
                        str(expected_prob * fidelity_threshold)
                        + (" 0" if cnf.computational_basis else ""),
                    ]
                )
                out_file = os.path.join(
                    cnf_file_root, f"{files_prefix}_{it_counter}_d4.out"
                )
                err_file = os.path.join(
                    cnf_file_root, f"{files_prefix}_{it_counter}_d4.err"
                )
                res_file = os.path.join(
                    cnf_file_root, f"{files_prefix}_{it_counter}_d4.res"
                )
                ...
                # Parse the result
                found, weight, assignment = get_result(
                    out_file, expected_prob, abs_value=expected_abs_value
                )
                if printing:
                    print(f"fidelity:{weight}")
                if weight == "CRASH":
        ...
        if printing:
            print(f"Run Time: {time.time() - start}")
        t_found, t_weight, t_assignment = get_result(
            out_file, expected_prob, expected_abs_value
        )
        if t_found:
            return (
                "FOUND",
                t_weight,
                cnf_copy.get_syn_qasm(t_assignment),
                cnf_copy.syn_gate_layer,
            )
        if bin_ub_results:
            return (
                str(error),
